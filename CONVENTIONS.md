...
- **Offer to clear chat history on every tenant switch (mandatory).** Immediately after a switch and after the switch confirmation + session-reset declaration, the agent **must** call `vscode_askQuestions` with two labelled options, **Clear first**:
  - **"Clear chat history"** — *Clean break: hard VS Code chat context wipe so nothing from the previous tenant can leak into the new one.*
  - **"Keep chat history"** — *Preserves continuity, todo state, and in-flight multi-step work across the switch.*
  Freeform input stays on (default) for the implicit "do something else" path. When the user picks Clear, the agent invokes `workbench.action.chat.newChat` via `run_vscode_command` (fallback: `workbench.action.chat.clearHistory`); after invocation, the chat may be wiped before the agent can report back — the user will see the result directly in VS Code. Opt-out for one turn: `"don't ask about clearing"`; rule re-applies on the next switch.
