...
**Clickable options for ALL user choices (mandatory, no exceptions).** Whenever the agent ends a turn by offering the user a choice — **including yes/no, this/that, and any two-option prompt** (e.g. "run it?", "keep going?", "want me to do A or B?", "should I X or skip?", "do you want me to also …?") — the agent **must** call `vscode_askQuestions` with labelled options instead of asking in plain prose. A mouse click is always faster than typing "yes". Always leave `allowFreeformInput` on (default); the freeform field is the implicit "other / type your own" option — do not also list a separate "Other" button. Cap labelled options at **6 maximum** (the freeform field counts toward that ceiling for visual budget); if you'd need more, narrow the question or split it. Plain-text prompts are reserved for open-ended questions (e.g. "what URL?", "paste the JSON") and explanations/recommendations the user is meant to read, not choose between. If unsure whether something counts as a choice: it does — use `vscode_askQuestions`. (See `CONVENTIONS.md` → *Agent Behavior*.)
...
3. Review **all** relevant workspace files (the active tenant's folder under `temp_dtctl_files/tenant-memory/<TENANTID>/` and any per-type subfolder relevant to the task, skills, `memories/repo/*`). Use `temp_dtctl_files/` only for tenant-specific experiments.
...
5. Apply the *Tool Selection* rubric below when picking between MCP and dtctl for a given task. Follow the Global Rule and all Agent-Agnostic DQL Rules below strictly. No tenant-specific names/IDs in root source files.
...
This workspace is an MCP repo at heart — it configures and ships skills/prompts/conventions on top of the Dynatrace MCP server (one local server launched from `.mcp.json` and `.vscode/mcp.json`). It is also configured **assuming the user has `dtctl` installed alongside MCP** so the agent can use the best of both.

**Tenant-data hard rule:** the only committed tenant routing is the public baseline (`demo.live` → `guu84124`). Any other tenant IDs/URLs must live in the local-only registry `temp_dtctl_files/tenant-memory/tenants.json` (gitignored). If MCP server entries are added for private tenants, they must be local-only and never committed.
...
- **Nickname display rule (mandatory).** Tenant nicknames may appear in agent output in **only two** situations: (1) the user invokes a switch (e.g. `"switch to <NICKNAME>"`) — the agent echoes the nickname back in the switch-confirmation line and in the `Using MCP server →` line; (2) the user asks which tenant the agent is connected to — the agent answers with **both paths broken out** so the user can see how each route is connected:
  ```
  dtctl context → <NICKNAME> · <TENANTID> · <class> · <safety>   (or "not configured")
  MCP server   → <NICKNAME> · <TENANTID>                          (or "not running / none selected")
  ```
  If only one path is configured, show that one and explicitly state the other is not configured. **Every other reference uses the raw 8-char `<TENANTID>` only**: MCP server lifecycle narration ("stopping MCP server for `<TENANTID>`…"), the dual-context echo block, session-reset declarations, status updates, error messages, internal narration about which servers are running, file paths, and any incidental mention of a tenant. The agent does **not** print other tenants' nicknames in prose, even briefly. When the agent must pass a server name to `workbench.mcp.start/stopServer` (the args field), it does so silently — the nickname stays inside the tool call, not in the user-visible message.
- **Single MCP server running at a time (mandatory).** Mirroring dtctl's single-context model, only one MCP server may be running at any moment. On every `"switch to <NICKNAME>"`:
  1. Stop every currently-running MCP server with `run_vscode_command` → `workbench.mcp.stopServer` (args: `["<server-name>"]`). Verified working from the agent. Narrate as "stopping MCP server for `<TENANTID>`" — never print the nickname here.
  2. If the target tenant has a matching MCP server entry in `.vscode/mcp.json` / `.mcp.json`, start it with `run_vscode_command` → `workbench.mcp.startServer` (args: `["<server-name>"]`). Note: tool visibility may lag a turn after start — the user can verify via *MCP: List Servers*.
  3. If the target tenant has no MCP server entry (dtctl-only tenant), leave **no** MCP server running. The agent operates dtctl-only for that tenant.
  4. If the target tenant has only an MCP server entry (no dtctl context), start that one server and operate MCP-only.
...
- **Clickable options for ALL user choices (mandatory).** Whenever the agent offers the user a choice — **including yes/no, this/that, and any two-option prompt** (e.g. "run it?", "keep going?", "A or B?", "X or skip?") — use `vscode_askQuestions` with labelled options. A mouse click is always faster than typing "yes". Always leave `allowFreeformInput` on; the freeform field is the implicit "other / type your own" option (do not add a separate "Other" button). Cap labelled options at **6 maximum** (freeform counts toward the visual budget). Plain text is reserved for open-ended prompts and for explanations/recommendations the user is meant to read, not choose between.
...
- **Never commit MCP config edits containing private tenants.** `.vscode/mcp.json` and `.mcp.json` are repo-tracked; keep them baseline-only in commits. Recommended local protection: `git update-index --skip-worktree .vscode/mcp.json .mcp.json`.
...
- **Mandatory dual-context echo before any tenant write.** Before any `dtctl apply`, MCP `update_*`/`create_*`/`send_*`, or write to a Dynatrace resource, emit one block on its own line and verify all three agree (IDs only, no nicknames per the *Nickname display rule*):
  ```
...
- **Offer to clear chat history on every tenant switch (mandatory).** Immediately after a switch (`"switch to <NICKNAME>"`, `switch dtctl to`, or `use the <NICKNAME> server`), and after the switch confirmation + session-reset declaration, the agent **must** call `vscode_askQuestions` with two labelled options, **Clear first**:
  - **"Clear chat history"** — *Clean break: hard context wipe so nothing from the previous tenant can leak into the new one.*
  - **"Keep chat history"** — *Preserves continuity, todo state, and in-flight multi-step work across the switch.*
  Freeform input stays on (default) for the implicit "do something else" path. The agent uses `run_vscode_command` with `workbench.action.chat.newChat` (or `workbench.action.chat.clearHistory` as fallback) when the user picks Clear; on Keep it proceeds normally. The user may opt out for a single turn with `"don't ask about clearing"` — the agent honors it but re-applies the rule on the next switch.
- **Don't name other tenants in findings.** In any answer about the active tenant, do not mention other tenant nicknames or IDs. Comparative shorthand ("same as `<other>`", "like we saw on `<other>`") is forbidden — describe each finding on its own terms. Other-tenant *IDs* are only allowed when they are the literal subject of the operation (registry/config edits, the dual-context echo block, or a comparison the user explicitly asked for); other-tenant *nicknames* are governed by the stricter *Nickname display rule* above and are effectively never printed outside a user-initiated switch or a "which tenant am I on?" answer. When in doubt, omit the other name.
...
- Review **all** relevant workspace files first (the active tenant's folder under `temp_dtctl_files/tenant-memory/<TENANTID>/`, skills, `memories/repo/*`).
